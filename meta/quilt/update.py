...
META_SERVER = "https://meta.quiltmc.org"
...
def fetch_and_update(game_version, loader_info):
    ...
    url_client = f"{META_SERVER}/v3/versions/loader/{game_version}/{loader_version}/profile/json"
...
def get_game_versions():
    return map(lambda v: v["version"], fetch(f"{META_SERVER}/v3/versions/game").json())

def get_loader_infos(game_version):
    return fetch(f"{META_SERVER}/v3/versions/loader/{game_version}").json()
